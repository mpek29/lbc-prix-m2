...
- **Do not hand-edit them to make a test pass.** If a test fails against a
  ...
  commit message, not a tweaked fixture.
- **Do not let Prettier or an editor reformat them.** Whitespace inside the
  markup is meaningful: the price is rendered with a U+00A0 before the euro
  sign, and normalising it would quietly delete the regression test for it.
  They are excluded in `.prettierignore` and `.gitattributes`.
...
`load.ts` reads them off disk and can splice several into a `<ul>`, which is how
tests build a full results list without a fifth capture to keep in sync.
